def gridify(x,y,val=None, grid=None, resolution=(100,100), value_approximation='min'):
    ...
    if grid is None:
        ...
        grid_x = grid[0]
        grid_y = grid[1]
        resolution = (len(grid_x),len(grid_y)) # NOTE: in this case resolution is coerced
        hdx = np.pad(np.diff(grid_x),(0,1),'symmetric') # diff with the last element duplicated
        ...
        crop = np.logical_or.reduce((x<np.min(xminbound), x>np.max(xmaxbound),
                                     y<np.min(yminbound), y>np.max(ymaxbound)))
        x = np.delete(x, crop)
        y = np.delete(y, crop)
        if val is not None:
    ...
    try:
        C = (np.logical_and.reduce((x[:,None,None] > xminbound,y[:,None,None] > yminbound,
                                    x[:,None,None] < xmaxbound,y[:,None,None] < ymaxbound)))
        ...
        if val is None or value_approximation == 'occupancy':
            ...
            D = np.sqrt(np.square(xx[None,:,:] - x[:,None,None]) + np.square(yy[None,:,:] - y[:,None,None]))
      ...
      print("Insufficient memory, splitting into %s x %s subproblems of resolution %s", 
            subdivisions[0], subdivisions[1], sub_resolutions)
      for sub_grid_x in np.split(xminbound[0,:],subdivisions[0]):
            for sub_grid_y in np.split(yminbound[:,0],subdivisions[1]):
                _, sub_values = gridify(x,y,val=val,
