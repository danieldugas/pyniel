...
class TextProgressBar:
    ...
    def update(self, batchmaker):
        ...
        if self.remaining_points is not None and self.max_ != max_:
            raise Warning("Maximum limit for progress has changed.\
        ...
        if self.remaining_points is None:
            ...
            self.remaining_points = list(np.round(1.0 * (np.arange(self.width) + 1)
                                                  * self.max_ / self.width))
    ...
    def abort(self):
        ...
        if missing < 0:
            raise ValueError("Negative missing points should not occur,\
            possible error within batchmaker. " + str(self.last_consumed))
        print("x" + " " * missing, end='')
        self.close_and_reset()
