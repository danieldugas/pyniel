...
class FCNetwork(object):
  def __init__(self, model_params):
    ...
    with tf.name_scope('Placeholders') as scope:
      self.input_placeholder = tf.placeholder(self.MP.FLOAT_TYPE,
                                              ...
                                              name="input")
      self.ground_truth_placeholder = tf.placeholder(self.MP.FLOAT_TYPE,
    ...
    for i, LAYER in enumerate(self.MP.FC_LAYERS):
      previous_layer, previous_layer_shape = self.build_FC_layer(LAYER, previous_layer, previous_layer_shape,
                                                                 'Layer'+str(i),
                                                                 'Layer'+str(i)+'Weights', '_layer_'+str(i),
                                                                 activation=tf.nn.relu)

    self.output, _ = self.build_FC_layer({'shape': self.MP.OUTPUT_SHAPE},
                                      previous_layer,
    ...
    with tf.name_scope('Loss') as scope:
      ...
      self.loss = tf.losses.softmax_cross_entropy(self.ground_truth_placeholder,
                                                  self.output)
  ...
  def train_on_single_batch(self, batch_input, batch_ground_truth,
                            train_target=None, cost_only=False, dropout=None, summary_writer=None):
    # feed placeholders
    dict_ = {self.input_placeholder: batch_input, self.ground_truth_placeholder: batch_ground_truth}
    if self.MP.DROPOUT is not None:
    ...
    if summary_writer is not None: summary_writer.add_summary(summary,
                                                              tf.train.global_step(self.sess, self.global_step_tensor))
    return np.array(cost)
  def cost_on_single_batch(self, batch_input, batch_ground_truth, summary_writer=None):
    return self.train_on_single_batch(batch_input, batch_ground_truth, train_target=self.zero,
